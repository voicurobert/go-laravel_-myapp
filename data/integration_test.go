...
func TestMain(m *testing.M) {
	...
	if err := pool.Retry(func() error {
		var err2 error
		testDB, err2 = sql.Open("pgx", fmt.Sprintf(dsn, host, port, user, password, dbName))
		if err2 != nil {
			return err2
		}
		return testDB.Ping()
...
func TestUser_Update(t *testing.T) {
	...
	if u.LastName != "Smith" {
		t.Error("lastname not updated in the database", err)
	}
}
...
func TestUser_ResetPassword(t *testing.T) {
	...
	if err == nil {
		t.Error("did not get error when trying to reset password for non existent user  ", err)
	}
}
...
func TestUser_Delete(t *testing.T) {
	...
	if err == nil {
		t.Error("retrieved user who was supposed to be deleted ", err)
	}
}
...
func TestToken_GetUserForToken(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to get user with a valid token", err)
	}
}
...
func TestToken_GetTokensForUser(t *testing.T) {
	...
	if len(tokens) > 0 {
		t.Error("tokens returned for non existent user ", err)
	}
}
...
func TestToken_Get(t *testing.T) {
	...
	if err != nil {
		t.Error("error when getting token by id ", err)
	}
...
func TestToken_GetByToken(t *testing.T) {
	...
	if err != nil {
		t.Error("error when getting token by token ", err)
	}

	_, err = models.Tokens.GetByToken("123")
	if err == nil {
		t.Error("error getting non existent token")
	}
}
...
func TestToken_ExpiredToken(t *testing.T) {
	...
	token, err := models.Tokens.GenerateToken(u.ID, -time.Hour*3)
	if err != nil {
		t.Error(err)
	}
	err = models.Tokens.Insert(*token, *u)
...
func TestToken_BadHeader(t *testing.T) {
	...
	token, err := models.Tokens.GenerateToken(id, time.Hour*3)
	if err != nil {
		t.Error(err)
	}
	...
	_, err = models.Tokens.AuthenticateToken(req)
	if err == nil {
		t.Error("failed to catch missing user for token")
	}
}
...
func TestToken_ValidToken(t *testing.T) {
	...
	if !okay {
		t.Error("valid token reported as invalid", err)
	}

	okay, _ = models.Tokens.ValidToken("abc")

	if okay {
		t.Error("invalid token reported as valid", err)
	}
	...
	if okay {
		t.Error("invalid token reported as valid for deleted token", err)
	}
}
